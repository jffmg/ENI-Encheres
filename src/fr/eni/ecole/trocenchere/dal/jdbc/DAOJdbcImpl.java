...
public class DAOJdbcImpl implements DAO {
	...
	private static final String SQL_GET_USER_ID_FROM_USER = "SELECT no_utilisateur FROM utilisateurs WHERE pseudo = ?";
	...
	private Article articleBuilder(ResultSet rs) throws SQLException {
		User user = new User(rs.getInt("no_utilisateur"), rs.getString("pseudo"), rs.getString("nom"),
		...
		Article article = new Article(rs.getString("nom_article"), rs.getString("description"),
				...
				rs.getInt("prix_initial"), rs.getString("etat_vente"), rs.getInt("no_categorie"), user,
	...
	public List<Article> displayArticles(String keyword, String category, HttpServletRequest request)
		...
		int categorySelected = categoryInt(category);
	...
	// method to prepare the Statement of basic Articles display
	public PreparedStatement basicDisplay(String keyword, int categorySelected, PreparedStatement pstmt,
			Connection cnx) {
	...
	// method in order to get user ID from user userName
	public int getUserId(String userName) {
		...
		try (Connection cnx = ConnectionProvider.getConnection()) {
			...
			pstmtID = cnx.prepareStatement(SQL_GET_USER_ID_FROM_USER);
			pstmtID.setString(1, userName);

			ResultSet res = pstmtID.executeQuery();

			System.out.println("Id de l'utilisateur : " + String.valueOf(res.getInt("no_utilisateur")));

			userID = res.getInt("no_utilisateur");
	...
	// method to change categoryString into categoryInt
	public int categoryInt(String category) {
